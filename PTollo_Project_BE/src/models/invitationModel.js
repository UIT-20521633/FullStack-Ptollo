...
const INVITATION_COLLECTION_SCHEMA = Joi.object({
  ...
  type: Joi.string()
    .required()
    //...Object.values(INVITATION_TYPES) - lấy ra tất cả các giá trị của object INVITATION_TYPES
  ...
  // Lời mời là room thì sẽ lưu thêm dữ liệu roomInvitation - optional
  roomInvitation: Joi.object({
    ...
    status: Joi.string()
      .required()
      .valid(...Object.values(ROOM_INVITATION_STATUS)),
  }).optional(),
...
const createNewRoomInvitation = async (data) => {
  try {
    const validData = await validateBeforeCreate(data);
    // Biến đổi một số dữ liệu liên quan tới ObjectId chuẩn chỉnh
    let newInvitationToAdd = {
    ...
    // Nếu tồn tại dữ liệu roomInvitation thì update cho cái
    if (validData.roomInvitation) {
      newInvitationToAdd.roomInvitation = {
        ...validData.roomInvitation,
        roomId: validData.roomInvitation.roomId,
      };
    }
    // Gọi insert vào DB
    const createdInvitation = await GET_DB()
...
// Query tổng hợp (aggregate) để lấy những bản ghi intivation thuộc về một  user cụ thể.
