...
const createNewBoardInvitation = async (reqBody, inviterId) => {
  try {
    ...
    //Lấy ra thông tin của cái invitation vừa tạo trả về cho FE
...
const getInvitations = async (userId) => {
  try {
    ...
    //   }
    // })
    const fillerInvitationsRoom = getInvitations.filter(
      (invitation) => invitation.type !== INVITATION_TYPES.ROOM_INVITATION
    );
    const resInvitations = fillerInvitationsRoom.map((i) => ({
      ...i,
...
const updateBoardInvitation = async (userId, invitationId, status) => {
  try {
    ...
    let boardId;
    if (getInvitation.type == INVITATION_TYPES.BOARD_INVITATION) {
      boardId = getInvitation.boardInvitation.boardId;
    } else if (getInvitation.type == INVITATION_TYPES.ROOM_INVITATION) {
      boardId = getInvitation.roomInvitation.boardId;
    } else {
      throw new Error(
        "boardId is undefined in both boardInvitation and roomInvitation"
      );
    }
    ...
    // Kiểm tra xem nếu status là ACCEPTED join board mà cái user (invitee) đã là owner hoặc member của board rồi thì trả về thông báo lỗi luôn.
    // Note: 2 mảng memberIds và ownerIds của board nó đang là kiểu dữ liệu ObjectId nên cho nó về String để check
    const boardOwnerAndMemberIds = [
      ...getBoard.ownerIds,
      ...getBoard.memberIds,
    ].toString();
    if (
      getInvitation.type == INVITATION_TYPES.BOARD_INVITATION &&
